...
if ( ! class_exists( 'WP_Bootstrap_Navwalker' ) ) :
	...
	class WP_Bootstrap_Navwalker extends Walker_Nav_Menu {
		...
		public function start_lvl( &$output, $depth = 0, $args = null ) {
			...
			$classes = array( 'dropdown-menu fs-6 p-0' );
			/**
			...
			$output .= "{$n}{$indent}<ul$class_names $labelledby style='left:auto;right:0'>{$n}";
		}
		...
		public function start_el( &$output, $item, $depth = 0, $args = null, $id = 0 ) {
			...
			if ( false !== strpos( $args->items_wrap, 'itemscope' ) && false === $this->has_schema ) {
				...
				$args->link_before = '<span itemprop="name" class="ms-2">' . $args->link_before;
			...
			if ( $this->has_children && 0 === $depth ) {
				...
				if ( $depth > 0 ) {
					$atts['class'] = 'dropdown-item text-end';
				} else {
					$atts['class'] = 'nav-link';
				}
			}
		...
		public function add_schema_to_navbar_ul( $args ) {
			if ( isset( $args['items_wrap'] ) ) {
				...
				if ( strpos( $wrap, 'SiteNavigationElement' ) === false ) {
					$args['items_wrap'] = preg_replace( '/(>).*>?\%3\$s/', ' itemscope itemtype=""$0', $wrap );
				}
			}
			return $args;
		}
